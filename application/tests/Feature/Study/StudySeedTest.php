...
it('seeds the exact fixed Wong Reang study through the database seeder', function () {
    ...
    expect(EvaluationPeriod::query()->get([
        ...
        'target_basis', 'consent_text', 'consent_data_description', 'consent_cookie_description',
        'consent_estimated_minutes', 'consent_withdrawal_description', 'research_contact',
        'fast_response_seconds', 'quality_rules_version', 'identical_answers_flag_enabled', 'instrument_version',
        ...
        $period->target_basis,
        $period->consent_text,
        $period->consent_data_description,
        $period->consent_cookie_description,
        $period->consent_estimated_minutes,
        $period->consent_withdrawal_description,
        $period->research_contact,
        $period->fast_response_seconds,
        $period->quality_rules_version,
        $period->identical_answers_flag_enabled,
        ...
        [
            ...
            'Penelitian ini mengevaluasi pengalaman pengguna pada modul Wong Reang Apps untuk menentukan prioritas perbaikan layanan.',
            'Jawaban UEQ mentah, waktu pengisian, urutan modul, dan profil kelayakan tanpa nama disimpan untuk analisis penelitian.',
            'Cookie anonim digunakan untuk mencegah modul yang sama dinilai dua kali.',
            10,
            'Partisipasi bersifat sukarela dan dapat dihentikan sebelum jawaban dikirim.',
            'peneliti@example.test',
            120,
            'quality-rules-v1',
            true,
            'UEQ-ID-26-v1',
